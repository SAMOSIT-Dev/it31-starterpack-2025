...
export default function Navbar() {
  return (
    <nav
      className="w-full h-[55px] px-[20px] bg-[#060404C2]/76 border border-[#3F2A2A] fixed top-0 z-20 flex justify-between items-center
    lg:h-[90px] xl:px-[65px] backdrop-blur-md"
    >
      <div className="flex justify-between items-center gap-[80px]">
        <Link to={"/"}>
          <img
            className="size-[46px] lg:size-[66px]"
            src={logo}
            alt="IT#31 Startter pack logo"
          />
        </Link>
        ...
        <div className="hidden lg:flex justify-between gap-[25px]">
          <Link to={"/course/houses"}>
            <button className="px-[20px] py-[10px] border border-white rounded-[20px] cursor-pointer">
              <p className="text-white font-light text-[20px] font-mitr">
                ตารางเรียน
              </p>
            </button>
          </Link>
